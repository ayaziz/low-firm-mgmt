...
export default function InvoiceDetailPage() {
  ...
  const handleFinalize = async () => {
    if (!invoice) return;
    await accountingApi.finalizeInvoice(id, invoice.row_version);
    load();
  };

  const handleSend = async () => {
    if (!invoice) return;
    await accountingApi.sendInvoice(id, invoice.row_version);
    load();
  };
  ...
  const handleVoid = async () => {
    ...
    await accountingApi.voidInvoice(id, reason, invoice.row_version);
    load();
  };
  ...
  const total = invoice.line_items?.reduce((s, li) => s + li.quantity * li.unit_price, 0)
      ...
      <Stack direction="row" alignItems="center" spacing={2} mb={3}>
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap">
          {invoice.status === 'Draft' && (
            <Button startIcon={<FinalizeIcon />} variant="contained" color="info" onClick={handleFinalize}>
              Finalize
            </Button>
          )}
          ...
          {['Sent', 'Overdue'].includes(invoice.status) && balance > 0 && (
            <Button startIcon={<PaymentIcon />} variant="contained" color="success" onClick={() => {
          ...
          {['Draft', 'Finalized', 'Sent'].includes(invoice.status) && (
            <Button startIcon={<VoidIcon />} variant="outlined" color="error" onClick={handleVoid}>
              Void
            </Button>
          )}
          <Button startIcon={<PdfIcon />} variant="outlined" onClick={handleDownloadPdf}>
            PDF
          </Button>
        </Stack>
      </Stack>
          ...
          <Card>
            ...
            <CardContent>
              <Stack spacing={1.5}>
                ...
                {invoice.due_date && (
                  <DetailRow label={t('accounting.dueDate')} value={new Date(invoice.due_date).toLocaleDateString()} />
                )}
                <DetailRow label={t('common.createdAt')} value={new Date(invoice.created_at).toLocaleString()} />
              </Stack>
              {balance < 0 && (
                <Alert severity="error" sx={{ mt: 2 }}>Overpayment detected!</Alert>
              )}
            </CardContent>
          </Card>
        </Grid>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right">{t('accounting.quantity')}</TableCell>
                    <TableCell align="right">{t('accounting.unitPrice')}</TableCell>
                ...
                <TableBody>
                  {invoice.line_items?.map((li, i) => (
                    <TableRow key={li.id || i}>
                      ...
                      <TableCell align="right">${li.unit_price.toFixed(2)}</TableCell>
                      <TableCell align="right">${(li.quantity * li.unit_price).toFixed(2)}</TableCell>
                    </TableRow>
                  ))}
                  <TableRow>
