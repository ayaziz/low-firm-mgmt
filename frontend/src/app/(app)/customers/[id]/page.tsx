...
export default function CustomerDetailPage() {
  ...
  const [financialSummary, setFinancialSummary] = useState<{ last_payment_date: string | null; overdue: number; paid: number; outstanding: number } | null>(null);
  ...
  return (
		<Box>
							...
							{customer.contacts.map((c: Contact, i: number) => (
								<React.Fragment key={c.id}>
									...
									>
										<ListItemText
											primary={c.name}
											secondary={`${c.role_id || ''} • ${c.phone || ''} • ${c.email || ''}`}
										/>
									</ListItem>
								</React.Fragment>
							))}
					...
					<Grid container spacing={3}>
						<Grid item xs={12} sm={4}>
							<Card>
								<CardContent>
									<Typography variant="caption" color="text.secondary">
										{t('accounting.overdue') || 'Overdue'}
									</Typography>
									<Typography variant="h5" fontWeight={700}>
										{financialSummary.overdue?.toLocaleString()}
									</Typography>
								</CardContent>
							</Card>
						</Grid>
			...
			>
				<DialogTitle>
					{t('common.edit')} {t('customer.title')}
				</DialogTitle>
				<DialogContent>
			...
			>
				<DialogTitle>
					{t('common.add')} {t('customer.contacts')}
				</DialogTitle>
				<DialogContent>
					<Stack spacing={2} mt={1}>
						<TextField
						...
						/>
						<TextField
							label={t('customer.contactRole')}
							fullWidth
							value={contactForm.contact_role}
							onChange={(e) =>
								setContactForm((f) => ({ ...f, contact_role: e.target.value }))
							}
						/>
						<TextField
			...
			>
				<DialogTitle>
					{t('common.add')} {t('customer.addresses')}
				</DialogTitle>
				<DialogContent>
					<Stack spacing={2} mt={1}>
						<TextField
				...
				<DialogActions>
					...
					<Button
						...
						disabled={saving || !addressForm.address_type || !addressForm.line1}
					>
						{t('common.save')}
					</Button>
				</DialogActions>
